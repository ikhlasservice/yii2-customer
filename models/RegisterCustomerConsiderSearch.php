...
/**
 * RegisterCustomerConsider represents the model behind the search form about `backend\modules\customer\models\RegisterCustomer`.
 */
class RegisterCustomerConsiderSearch extends RegisterCustomer
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            ...
            [['data', 'doc', 'doc_fully', 'doc_because', 'score', 'seller_receive', 'seller_receive_because', 'seller_date', 'staff_receive', 'staff_date', 'staff_receive_because','fullname'], 'safe'],
    ...
    public function search($params)
    {
        ...
        if (Yii::$app->user->can('seller')) {
            $query->where(['seller_id' => Yii::$app->user->id, 'status' => [2]]);
